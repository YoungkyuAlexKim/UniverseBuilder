...
아래 정보를 바탕으로, '기존 설정 문장'의 핵심 영혼과 의도를 **완벽히 보존**하면서, 독자가 상상의 나래를 펼치고 그 세계에 몰입할 수 있는 매력적인 설정으로 승화시켜 주세요.
...
"""
    ...
    # [신규] 장면 생성을 위한 프롬프트 템플릿 3종 추가
    scene_generation_novel: str = """당신은 독자의 감각을 자극하고 마음을 움직이는 묘사에 능한 베테랑 소설가입니다.
주어진 상황과 설정을 바탕으로, {character_names}의 감정과 행동, 그리고 주변 분위기가 생생하게 드러나는 한 편의 장면을 소설 형식으로 서술해주세요.
독자가 마치 그 장면 속에 직접 들어가 있는 듯한 몰입감을 선사해야 합니다. 다른 설명 없이, 오직 소설 본문만 작성해주세요.

---
[창작의 재료]
- 세계관 장르: {worldview_genre}
- 세계관의 규칙: {worldview_rules}
- 이야기 전체 요약: {scenario_summary}
- 이 장면의 핵심 내용(플롯): {plot_title} - {plot_content}
- 주요 등장인물 정보:
{characters_context}
---
[결과물]
...
카메라가 무엇을 비춰야 할지, 배우가 어떤 톤으로 말해야 할지가 눈에 보이도록 구체적으로 작성해야 합니다.
...
주어진 상황과 설정을 바탕으로, {character_names} 사이의 대화와 최소한의 상황 묘사로 이루어진 게임 대사 지문을 작성해주세요.
각 대사는 캐릭터의 성격을 명확히 드러내고, 플레이어가 이야기에 흥미를 느끼도록 만들어야 합니다.
