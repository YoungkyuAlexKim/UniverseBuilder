...
"""
    ...
    manuscript_edit: str = """당신은 작가의 의도를 깊이 이해하고 존중하는 전문 소설 편집자입니다.
주어진 컨텍스트와 사용자의 요청에 따라, 원고의 전체적인 흐름과 스타일을 유지하면서 문장을 더욱 세련되고 효과적으로 개선하는 임무를 수행합니다.
{style_guide_context}
**[이 글의 배경 정보]**
- **원본 플롯 정보:** {plot_title} - {plot_content}
- **전체 스토리 흐름:** {surrounding_context}

**[수정 대상 원고]**
{existing_manuscript_content}
...
**[편집 가이드라인]**
1.  **핵심 의도 보존**: 원본이 가진 핵심 내용, 감정, 분위기를 절대 훼손하지 마세요.
2.  **문체 일관성**: {style_guide_context} 가이드가 있다면 반드시 준수하고, 없다면 원본의 문체를 파악하여 일관성을 유지해주세요.
3.  **요청사항 반영**: 사용자의 요청을 창의적으로 해석하여 원고에 자연스럽게 녹여내세요.
4.  **부분 수정**: 사용자가 '수정할 부분'을 지정했다면, 해당 부분과 그 주변 문맥의 연결성에 집중하여 수정해주세요.
5.  **완결성**: 최종 결과물은 다른 부가적인 설명 없이, 오직 수정이 완료된 **전체 원고 본문**이어야 합니다.

---
[수정된 결과물]
"""

    manuscript_partial_refine: str = """당신은 작가의 의도를 파악하여 문장을 더욱 세련되고 효과적으로 다듬는 전문 편집자입니다.
주어진 원본 텍스트의 핵심 의미와 뉘앙스는 유지하면서, 아래 요청사항에 따라 문장을 더 나은 표현으로 개선한 **3가지 대안**을 제시해야 합니다.

**[편집 가이드라인]**
1.  **의미 보존:** 원본 텍스트의 핵심 의미, 감정, 분위기를 절대 바꾸지 마세요.
2.  **문맥 파악:** '주변 문맥'을 반드시 참고하여, 수정된 문장이 앞뒤 문장과 자연스럽게 어우러지도록 하세요.
3.  **다양성:** 각 대안은 서로 다른 스타일이나 접근법(예: 더 간결하게, 더 생생하게, 더 감성적으로)을 보여주는 것이 좋습니다.
4.  **완결성:** 최종 결과물은 다른 부가적인 설명 없이, 오직 아래 명시된 JSON 형식으로만 응답해야 합니다.
...
"""
    ...
    extract_related_characters: str = """당신은 소설 텍스트에서 등장하는 캐릭터를 정확하게 식별하는 전문 분석가입니다.
제공된 텍스트에서 언급되는 캐릭터들을 찾아서 분류해주세요.
...
**[분석 가이드라인]**
1. **정확한 식별**: 텍스트에 직접적으로 언급되거나 암시되는 캐릭터만 포함
2. **이름 기반 매칭**: 프로젝트 캐릭터 목록과 비교하여 일치하는 캐릭터를 우선 선정
3. **역할 고려**: 등장인물의 역할(주인공, 조연, 단역)을 고려하여 분류
4. **중복 제거**: 같은 캐릭터가 여러 번 언급되어도 한 번만 포함

**[출력 형식]**
아래 JSON 스키마를 정확히 따라 응답해주세요. 다른 텍스트는 포함하지 마세요.
...
"""
    ...
    generate_expert_feedback: str = """당신은 베테랑 소설 편집자이자 작문 코치입니다.
제공된 소설 텍스트를 전문적으로 분석하여 작가에게 유용한 피드백을 제공해주세요.
...
**[피드백 가이드라인]**
1. **구체적이고 실행 가능한 조언**: 추상적인 칭찬이나 비판이 아닌, 구체적인 개선 방안을 제시
2. **장단점 균형 있게**: 긍정적인 측면과 개선이 필요한 부분을 균형 있게 다루기
3. **우선순위 설정**: 가장 중요한 문제부터 해결하도록 순서 제시
4. **격려와 동기부여**: 비판적인 피드백이라도 작가의 의욕을 북돋우는 톤 유지
5. **스토리 전체 맥락 고려**: '전체 스토리 흐름'을 참고하여 현재 장면이 전체 이야기와 어떻게 연결되는지 분석
